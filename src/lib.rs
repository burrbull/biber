...
impl<L: Eq + Hash, R: Eq + Hash> BiSet<L, R> {
    ...
    RQ: Eq + Hash + ?Sized,
  {
    self
      ...
      .map(|s| s.contains(Wrapper::wrap(right)))
      .unwrap_or(false)
  }
    ...
    Q: Eq + Hash + ?Sized,
  {
    OptIter::new(
      self
        .rl
        .get(Wrapper::wrap(right))
        .map(|e| e.iter().map(|r| &*r.0)),
    )
  }
}
...
pub trait EasyMap<K, V> {
  fn in_entry<'a, 'b, Q>(&'a mut self, key: &'b Q) -> &'a mut V
  where
...
impl<K, V> EasyMap<K, V> for HashMap<K, V> {
  fn in_entry<'a, 'b, Q>(&'a mut self, key: &'b Q) -> &'a mut V
  where
    Q: Hash + hashbrown::Equivalent<K> + ?Sized,
    K: Eq + Hash + From<&'b Q>,
    V: Default,
  {
    self.entry_ref(key).or_default()
  }
}

pub trait NestedMap<K1, K2, V> {
  fn contains_key2<Q1: ?Sized, Q2: ?Sized>(&self, k1: &Q1, k2: &Q2) -> bool
  where
    K1: Borrow<Q1>,
    Q1: Hash + Eq,
    K2: Borrow<Q2>,
  ...
  where
    ...
    K2: Borrow<Q2>,
    Q2: Hash + Eq;
  fn get_mut2<Q1: ?Sized, Q2: ?Sized>(&mut self, k1: &Q1, k2: &Q2) -> Option<&mut V>
  where
    K1: Borrow<Q1>,
    Q1: Hash + Eq,
    K2: Borrow<Q2>,
    Q2: Hash + Eq;
  fn in_entry2<'a, 'b, Q1, Q2>(&'a mut self, key1: &'b Q1, key2: &'b Q2) -> &'a mut V
  where
    Q1: Hash + hashbrown::Equivalent<K1> + ?Sized,
    Q2: Hash + hashbrown::Equivalent<K2> + ?Sized,
    K1: Eq + Hash + From<&'b Q1>,
    K2: Eq + Hash + From<&'b Q2>,
    V: Default;
}
...
impl<K1: Hash + Eq, K2: Hash + Eq, V> NestedMap<K1, K2, V> for HashMap<K1, HashMap<K2, V>> {
  ...
  where
    ...
    V: Default,
  {
    self.in_entry(key1).in_entry(key2)
  }
}
