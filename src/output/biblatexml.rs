...
impl BibLaTeXML {
  ...
  fn set_output_target_file(self, $toolfile, $init) {
    ...
    if ($init) {
      ...
      $xml->xmlDecl();
      $xml->pi("xml-model", format!("href=\"{schemafil}\" type=\"application/xml\" schematypens=\"http://relaxng.org/ns/structure/1.0\""));
  ...
  fn set_output_entry(&mut self, be: &Entry, section: &Section, dm: &DataModel) {
    let bee = be.get_field("entrytype").unwrap();
    let $dmh = crate::config::get_dm_helpers();
    let $secnum = $section->number;
    let key = be.get_field("citekey").unwrap();
    ...
    for namefield in dm.get_fields_of_type(FieldType::List, &[DataType::Name], None) {
      ...
      if (let $nf = $be->get_field($namefield)) {
        ...
        let attrs = attribute_map!("type" => namefield);

        // Did we have "and others" in the data?
        if nf.get_morenames() {
          attrs.insert("morenames", 1);
        }
        ...
        for nlo in CONFIG_OPT_SCOPE_BIBLATEX.iter_by_right("NAMELIST") {
          if (defined($nf->${\"get_$nlo"})) {
            ...
            if ($CONFIG_BIBLATEX_OPTIONS{NAMELIST}{$nlo}{OUTPUT}) {
              attrs.insert(nlo, map_boolean(nlo, nlov, "tostring"));
            }
          }
        }
    ...
    for field in dm.get_fields_of_type(FieldType::Field,
      ...
      if (length($val) || // length() catches '0' values, which we want
        ...
        $xml->dataElement([$xml_prefix, $field], NFC(val), AttributeMap::new());
      }
    }
    ...
    for datefield in dm.get_fields_of_datatype(&[DataType::Date]) {
      ...
      if (let $sf = $be->get_field("${d}year") ) { // date exists if there is a year
        ...
        push @start,
          grep {$_}
            $sf,
              date_monthday($overridem || be.get_field(format!("{d}month"))),
                date_monthday($overrided || be.get_field(format!("{d}day")));
        push @end,
          grep {defined($_)} // because end can be def but empty
            be.get_field(format!("{d}endyear")),
              date_monthday($overrideem || be.get_field(format!("{d}endmonth"))),
                date_monthday(be.get_field(format!("{d}endday")));
        // Date range
        if (@end) {
          let mut start = NFC(start.join("-"));
          let mut end = NFC(end.join("-"));

          // If start hour, there must be minute and second
          if let Some(sh) = be.get_field(format!("{d}hour")) {
            start.push_str(NFC(format!(
                "T{sh:.2}:{:.2}:{:.2}",
          ...
          if let Some(eh) = be.get_field(format!("{d}endhour")) {
            end.push_str(NFC(format!(
              "T{eh:.2}:{:.2}:{:.2}",
              be.get_field(format!("{d}endminute")),
              be.get_field(format!("{d}endsecond"))
            )));
          }
