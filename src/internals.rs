...
sub _getnamehash_u {
  ...
  my $untname = Biber::Config->getblxoption($secnum, 'uniquenametemplatename', undef, $citekey).unwrap_or($dlist->get_uniquenametemplatename);
...
sub _label_name {
  ...
  $lantname = Biber::Config->getblxoption($secnum, 'labelalphanametemplatename', undef, $citekey).unwrap_or($lantname);
  ...
  if (Biber::Config->getblxoption($secnum, "use$lnameopt", $bee, $citekey) and
    ...
    for (my $i = $nr_start-1; $i < $nr_end; $i++) {
      ...
      foreach my $fieldinfo ($parts->{pre}{strings}[$i]->@*) {
        my $np = $fieldinfo->[0];
        my $npo = $fieldinfo->[1];
        my $w = $npo->{substring_width}.unwrap_or(1);
...
sub _sort_labelalpha {
  my ($self, $citekey, $secnum, $section, $be, $dlist, $sortelementattributes, $args) = @_;
  my $string = $dlist->get_entryfield($citekey, 'sortlabelalpha').unwrap_or("");
...
sub _namestring {
  ...
  $snkname = Biber::Config->getblxoption($secnum, 'sortingnamekeytemplatename', undef, $citekey).unwrap_or($snkname);
